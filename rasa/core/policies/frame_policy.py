...
from rasa.core.actions.action import ACTION_CHANGE_FRAME_NAME
...
class FrameMemoizationPolicy(AugmentedMemoizationPolicy):
    ...
    def predict_action_probabilities(
        ...
        logger.debug(f"Number of frames: {len(tracker.frames)}")
        logger.debug(f"Frames: {tracker.frames}")
        ...
        if isinstance(events[idx], UserUttered):
            self.original_prediction = super().predict_action_probabilities(
                tracker, domain
            )
        ...
        if (
            ...
            and events[idx].action_name == ACTION_CHANGE_FRAME_NAME
        ):
